<?php  
    ...
    $userID = $_SESSION['userID']; // UserID moet al bekend zijn.
    $array_ber = []; //alle bericht-gegevens + naam werkgever

    //SQL-query om alle berichten en werkgevers-naam op te vragen in phpmyadmin
    $sql = "SELECT verstuurd_werknemer.titel, verstuurd_werknemer.datum, verstuurd_werknemer.bericht, verstuurd_werknemer.gelezen, verstuurd_werknemer.ID, werkgevers.naam, werkgevers.id, ID_vacature
            FROM verstuurd_werknemer 
            INNER JOIN werkgevers
            ON werkgevers.ID = verstuurd_werknemer.ID_werkgever AND verstuurd_werknemer.ID_werknemer=".$userID."
